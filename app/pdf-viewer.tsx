...
// Worker dari file lokal di /public
...
export default function PDFViewer() {
  ...
  // ── Deteksi ukuran layar & container
  useEffect(() => {
    const measure = () => {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === '-') setScale(s => Math.max(+(s - 0.15).toFixed(2), 0.5));
  ...
  return (
    <>
      <style>{`
        ...
        .react-pdf__Page__canvas { max-width:100% !important; height:auto !important; box-shadow:0 4px 32px rgba(0,0,0,0.5); }
      ...
      <div id="vwr-root" ref={rootRef}>
        ...
        <header id="vwr-bar" style={{
          ...
          <div style={{ display:'flex', alignItems:'center', gap:3 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:4 }}>
              <input
                ...
                inputMode="numeric" aria-label="Halaman"
                style={{
          ...
          <div className="hide-sm" style={{ display:'flex', alignItems:'center', gap:3 }}>
            <Btn onClick={() => setScale(s => Math.max(+(s-0.15).toFixed(2),0.5))} disabled={scale<=0.5} title="Perkecil"><Ic.zoomOut/></Btn>
            <button onClick={() => setScale(1.0)} style={{
        ...
        <main id="vwr-main" ref={mainRef}>
          ...
          {isLoading && (
            <div style={{
              ...
              <div style={{ width:44, height:44, border:'3px solid rgba(99,102,241,0.2)', borderTopColor:'#6366f1', borderRadius:'50%' }} className="pdf-spin"/>
              <p style={{ color:'#5a6478', fontSize:13 }}>Memuat PDF...</p>
            </div>
          )}
          ...
          <Document
            ...
            loading={null}
            error={
              <div style={{ padding:32, textAlign:'center', color:'#8892a4', display:'flex', flexDirection:'column', alignItems:'center', gap:12 }}>
